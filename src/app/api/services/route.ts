...
export async function DELETE(req: Request) {
  ...
  const sql = getSql();
  try {
    const del = (await sql`DELETE FROM services WHERE id = ${id}::uuid RETURNING id`) as { id: string }[];
    if (!del[0]) {
    ...
    if (code === "23503") {
      return NextResponse.json(
        { error: "Cannot delete service because it is referenced by other records (e.g. appointments)." },
        { status: 409 }
      );
    }
    throw e;
